odoo.define('rma_portal_form_validation_ept', function (require) {
    ...
    $(document).ready(function () {
        ...
        $(".return_qty").keyup(function () {
            ...
            var delivered_qty = parseInt($(this).parents(".col-md-2").siblings().find("#delivered_qty").val());
            checkbox_validate()
            ...
            if (return_qty < 0) {
                $(this).parents(".col-md-2").siblings().find("input[type=checkbox]").prop("checked", false);
                $(this).parents(".col-md-2").siblings().find("input[type=checkbox]").attr("required", "true")
            ...
            if (return_qty >= delivered_qty) {
                ...
                $(this).parents(".col-md-2").siblings().find("input[type=checkbox]").prop("checked", true);
                $(this).parent().siblings().find("select").attr("required", "true")
                $(this).parent().siblings().find("input[type=text]").attr("required", "true")
            }
	         if (return_qty < delivered_qty) {
	        	 $(this).parents(".col-md-2").siblings().find("input[type=checkbox]").prop("checked", true);
	             $(this).parents(".col-md-2").siblings().find("select").attr("required", "true")
	            }
