...
class crm_Claim_ept(models.Model):
    ...
    section_id = fields.Many2one('crm.team', 'Sales Channel', index=True,
    ...
    picking_id = fields.Many2one('stock.picking',
                                 string='Delivery Order')  # ,domain="[('state','=','done'),('picking_type_id.code','=','outgoing'),('sale_id','!=',False)]")
    ...
    state = fields.Selection(
        [('draft', 'Draft'), ('approve', 'Approved'), ('process', 'Processing'), ('close', 'Closed'),
    ...
    rma_image = fields.Binary(attachment=True,string="Attachment")
    ...
    def create(self, vals):
        ...
        if not self.env['mail.followers'].search(
                [('res_id', '=', res.id), ('res_model', '=', 'crm.claim.ept'), ('partner_id', '=', res.partner_id.id)]):
    ...
    def action_rma_send_email(self):
        if self.state == 'close':
            email_template = self.website_id.rma_close_mail_template or self.env.ref('rma_ept.rma_close_mail_template', False)
        elif self.state == 'approve':
            email_template = self.website_id.rma_approve_mail_template or self.env.ref('rma_ept.rma_approve_mail_template', False)
        elif self.state == 'reject':
            email_template = self.website_id.rma_reject_mail_template or self.env.ref('rma_ept.rma_reject_mail_template', False)
        
        mail_mail = email_template and email_template.with_context(rma_id=self.id).send_mail(self.id) or False
...
#         refund_invoice_ids_rec and self.write({'refund_invoice_ids': [(6, 0, refund_invoice_ids_rec)]})
    ...
    def _message_auto_subscribe_notify(self, partner_ids, template):
        if self._name =='crm.claim.ept' and template == 'mail.message_user_assigned':
            return
        return super(crm_Claim_ept, self)._message_auto_subscribe_notify(partner_ids=partner_ids,template=template)
