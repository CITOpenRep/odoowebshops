        ...
        if slider_obj:
            if slider_obj.slider_type == 'product':
                ...
                if tmplt:
                    ...
                    return False
            elif slider_obj.slider_type == 'latest_merch':
                domain=[('website_published','=',True)]
                latest_merch = request.env['product.template'].sudo().search(domain,limit=slider_obj.slider_limit,order='create_date DESC')
                return request.env.ref("website_sale_customisation.latest_merch").render({'latest_merch':latest_merch})
            else:
...
class emiproThemeBaseExtended(WebsiteSaleWishlist):
    ...
    def rm_from_wishlist(self, wish, **kw):
        ...
        remain_wish = request.env['product.wishlist'].sudo().search_count(
            [('partner_id', '=', request.env.user.partner_id.id), ('active', '=', True)])
        if remain_wish:
            return remain_wish
        
    #if wishlist is empty then display the message  
    ...
    @http.route(['/shop/wishlist/count'], type='json', auth="public", website=True)
    def get_wishlist_count(self, count=False, **kw):
        values = request.env['product.wishlist'].with_context(display_default_code=False).current()
