...
sAnimations.registry.ProductWishlist = sAnimations.Class.extend(ProductConfiguratorMixin, {
     ...
     */
    willStart: function () {
        //ADDED THE SCRIPT TO OVERRIDE THE WILLSTART METHOD
       ...
       var wishDef =  ajax.jsonRpc('/shop/wishlist/count', 'call', {
	    }).then(function (res) {
    ...
    _updateWishlistView: function () {
        if (this.wishlistProductIDs.length > 0) {
            ...
            //Changes to changes the color of wish list number
            // Added By Preet Bhatti
            //Date: 28/may
            if($('.my_wish_quantity').hasClass('badge-primary')) {
                $('.my_wish_quantity').toggleClass('badge-primary badge-success')
            }
        } else {
            $('#my_wish').hide();
        }
